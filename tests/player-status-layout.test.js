...
const manifest = read('tools/entry-sources.v067.json');
...
assert.ok(header.includes('getImageData'));
assert.ok(header.includes('createCanvas(avatarKey, 104, 104)'));
assert.ok(header.includes("context.fillStyle = '#b88978'"));
assert.ok(header.includes('context.drawImage('));
assert.ok(header.includes('maskShape.fillCircle(50, 50, 24)'));
assert.ok(header.includes('createAvatarTexture'));
