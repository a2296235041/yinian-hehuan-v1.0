...
assert.ok(source.includes('scene.add.zone'));
assert.ok(source.includes('return { container, hitTarget, text, layout }'));
assert.ok(source.includes("const MIN_HEIGHT = 96"));
assert.ok(uiScene.includes('Game.TransitionMessageBox.create(this)'));
assert.ok(uiScene.includes("this.logBox.hitTarget.on('pointerdown'"));
assert.ok(uiScene.includes('duration: 80'));
