/* release 0.1.4 */ (function installExplorationDOMController(root) {
  'use strict';
  ...
  function handleSubmit(text) {
    if (session?.result) return continueConversation(text);
    return exploreCurrent(text);
  }
  ...
  async function continueConversation(text) {
    ...
    root.GameExplorationPanel.setBusy(true, root.GameTrafficSaver.isEnabled('exploration')
      ? '' : 'AI 正在回应，预计数秒…');
    try {
