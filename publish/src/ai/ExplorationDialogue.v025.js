(function installExplorationDialogue(root) {
  ...
  async function run(session, instruction, fallback, onUpdate) {
    ...
    if (root.GameTrafficSaver.isEnabled('exploration')) {
      const fixedText = root.GameTrafficSaver.nextText('exploration');
      onUpdate?.(fixedText);
      return { text: fixedText, failed: false, source: 'fixed' };
    }
    let finalText = '';
    try {
  ...
  async function describe(session, result, onUpdate) {
    ...
    const body = generated.text === fact ? fact
