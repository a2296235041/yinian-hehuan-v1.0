/* release 0.1.6 */ (function installAIService(root) {
  'use strict';
  ...
  async function sendMessage(text, options = {}) {
    ...
    trace('send-received', { activeSession: Boolean(current), inputLength: content.length,
      busy: completions.isBusy(), interaction: Boolean(options.displayContent) });
    if (!current || !content) {
      const reason = !current ? 'no-current-session' : 'empty';
      trace('send-blocked', { reason });
      if (!current) emitStatus('error', '对话会话已失效，请返回后重新打开 NPC 对话。');
      return { ok: false, reason };
    }
    if (completions.isBusy()) {
      trace('send-blocked', { reason: 'completions-busy' });
      emitStatus('busy', '上一条回复仍在生成，请稍候。');
      return { ok: false, reason: 'completions-busy' };
    }
    ...
    try {
      ...
      trace('request-result', { source: result.source || '',
      ...
      if (current !== session) return { ok: false, reason: 'stale-session' };
      trace('request-error', { code: error?.code || '',
        category: error?.category || '', message: error?.message || 'unknown' });
