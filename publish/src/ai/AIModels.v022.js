(function installAIModels(root) {
  ...
  const PREFERRED_DIALOGUE_MODEL = 'Dash-0727-128k';
  const PREFERRED_DRAW_MODEL = 'iroha';
  ...
  let state = {
    ...
    drawModel: PREFERRED_DRAW_MODEL,
    recommendedModel: PREFERRED_DIALOGUE_MODEL
  };
  ...
  function chooseRecommended(models, platformDefault) {
    const preferred = models.find((model) => (
      model.internalName.toLowerCase() === PREFERRED_DIALOGUE_MODEL.toLowerCase()
    ));
  ...
  async function loadDrawModels() {
    try {
      ...
      let selected = fallback.some((model) => model.id === PREFERRED_DRAW_MODEL)
        ? PREFERRED_DRAW_MODEL
  ...
  root.GameAIModels = {
    ...
    getDrawModel: () => state.drawModel || PREFERRED_DRAW_MODEL,
    getState: snapshot
  };
}(window));
