/* release 0.1.4 */ var Game = window.Game || {};
...
Game.Scenes.ExplorationScene = class ExplorationScene extends Phaser.Scene {
    ...
    handleSubmit(text) {
        if (this.session?.result) return this.continueConversation(text);
        return this.exploreCurrent(text);
    }
    async continueConversation(text) {
        ...
        GameExplorationPanel.setBusy(true, window.GameTrafficSaver.isEnabled('exploration') ? '' : 'AI 正在回应，预计数秒…');
        try {
    ...
    async exploreCurrent(intent) {
        ...
        try {
            ...
            GameExplorationPanel.setBusy(true, window.GameTrafficSaver.isEnabled('exploration') ? '' : 'AI 正在生成探索内容，预计数秒…');
